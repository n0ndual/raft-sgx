...
{
    int success;
  //没必要知道谁是raft leader，收到消息，回复消息即可
    /* leader's Raft port */
    int leader_port;
...
struct peer_connection_s
{
    ...
    // 实现时所做的一个优化，记录了一个AppendEntries中的数量？
    // 在raft算法中本身不需要
    // 只是在通信时需要的
...
typedef struct
{
    ...
    // 不应该锁，收到client一个log，就放进自己的logs里边即可
...
static int __http_get_id(h2o_handler_t *self, h2o_req_t *req)
{
    ...
    // 这里用到leader，记录leader的目的只是为了转发处理client的读写请求？
    if (!leader)
...
{
  // 在raft_add_node中设置user_data为peer_connection
    ...
    )
{
  //为什么长度是3
    ...
    // 消息通信时，多发一个entries的数量，以方便处理？
    msg.ae.n_entries = m->n_entries;
    // 这里只是序列化，没有send。还有_peer_msg_send方法
...
{
    tpl_bin tb;
    // tpl_peek 返回一个format，其实可以把它写成静态的
...
static int __deserialize_and_handle_msg(void *img, size_t sz, void *data)
{
    ...
    case MSG_HANDSHAKE:
    {
        ...
        // handshake 只是leader与follower之间的handshake？
        if (!leader)
        {
            return __send_handshake_response(conn, HANDSHAKE_FAILURE, NULL);
        }
        // 本节点不是leader，告诉他leader是谁
        else if (raft_node_get_id(leader) != sv->node_id)
        {
                                             ...
                                             const int status)
{
    ...
    // 新peer的tcp连接建立后，直接去handshake要leader？
    ...
    )
{
    ...
    ety->data.buf = malloc(val.mv_size);
    memcpy(ety->data.buf, val.mv_data, val.mv_size);
